...
$insert = $db->prepare('INSERT INTO getmethod (`c1`, `c2`, `c3`) VALUES (:c1,:c2,:c3)');
$insert->execute([ ':c1' => $content1, ':c2' => $content2, ':c3' => $content3 ]);

if($insert && isset($_GET["c1"]) && isset($_GET["c2"]) && isset($_GET["c3"])) {
  echo "1";
} else {

$dataMAX = $db->prepare('SELECT MAX(`id`) AS LargestID FROM `getmethod`');
$dataMAX->execute();
$dataMAXID = $dataMAX->fetch(\PDO::FETCH_ASSOC);
$maxID = $dataMAXID["LargestID"];
$maxID -= 500;

$dataPrepare = $db->prepare('SELECT * FROM `getmethod` WHERE (id>:id) ORDER BY id DESC');
